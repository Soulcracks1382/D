...
void usage() {
    printf("Welcome to black hat server\n\nUsage following format:\n•→ ./blackhat <ip> <port> <duration> <threads>\n\nTelegram: @BlackHatDDoS\n");
    exit(1);
}
...
int main(int argc, char *argv[]) {
    ...
    printf("Flood started on %s:%d for %d seconds with %d threads Telegram @BlackHatDDoS\n", ip, port, time, threads);
    ...
    for (int i = 0; i < threads; i++) {
        ...
        printf("Launched thread with Telegram @BlackHatDDoS: %lu\n", thread_ids[i]);
    }
    ...
    free(thread_ids);
    printf("Attack finished with Telegram @BlackHatDDoS\n");
    return 0;
}
